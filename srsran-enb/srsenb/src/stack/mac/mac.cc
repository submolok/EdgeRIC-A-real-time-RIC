...
void mac::calFairness_dl(sched_interface::dl_sched_res_t sched_result_dl)
{
  ...
  for (uint32_t k = 0; k < sz2; k++) {
    uint16_t rnti = sched_result_dl.data[k].dci.rnti;

    for (uint32_t tb_ = 0; tb_ < SRSRAN_MAX_TB; tb_++) {
      // printf("%i %i transmitted bytes %i \n", k, tb_, sched_result.data[k].tbs[tb_]);
      tx_bytes += sched_result_dl.data[k].tbs[tb_];
      tx_bytes_ues[rnti] += sched_result_dl.data[k].tbs[tb_];
    }
  }
  
}

void mac::calFairness_ul(sched_interface::ul_sched_res_t sched_result_ul)
{
  uint32_t rx_bytes = 0;
  ...
  for (uint32_t k = 0; k < sz; k++) {
    uint16_t rnti = sched_result_ul.pusch[k].dci.rnti;
    rx_bytes += sched_result_ul.pusch[k].tbs;
    rx_bytes_ues[rnti] += sched_result_ul.pusch[k].tbs;
  }
